...
class PinchImageView : android.support.v7.widget.AppCompatImageView {
    ...
    private val mGestureDetector =
        GestureDetector(this@PinchImageView.context, object : GestureDetector.SimpleOnGestureListener() {
    ...
    fun getInnerMatrix(matrix: Matrix?): Matrix {
        ...
        if (isReady) {
            //原图大小
            val tempSrc =
                MathUtils.rectFTake(0f, 0f, drawable.intrinsicWidth.toFloat(), drawable.intrinsicHeight.toFloat())
            //控件大小
     ...
     * @param duration 动画时间
     */
    @JvmOverloads constructor(start: Matrix, end: Matrix, duration: Long = SCALE_ANIMATOR_DURATION.toLong()) :
